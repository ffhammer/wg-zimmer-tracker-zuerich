...
def fetch_listing(url: str) -> Optional[StudentsCHListing]:
    response = requests.get(url)
    if not response.ok:
        logger.error(f"Received {response.status_code} for {url}")
        return
    try:
        return parse_html(response.content, url)
    except Exception as e:
        logger.error(f"failed parsing '{url}' with {e}")


def parse_html(html: str, url: str) -> StudentsCHListing:
...
def fetch_all_listings(
    ...
    try:
        ...
        urls = ["https://www.students.ch" + a["href"] for a in links]
        listings = [
            l
            for u in tqdm(urls, desc="Fetching Individual Listings")
            if (l := fetch_listing(u))
        ]
        for l in listings:
            l.first_seen = now
        return listings
