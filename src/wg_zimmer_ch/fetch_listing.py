...
def fetch_listing(url: HttpUrl, now: WGZimmerCHListing) -> WGZimmerCHListing | None:
    """
    ...
    try:
        ...
        img_urls = list(
            {
            ...
            }.union(
                {
                    ...
                    if img.get("src")
                    and img["src"].startswith("/docroot/img.wgzimmer.ch")
                }
            ).difference(
                ("https://www.wgzimmer.ch/docroot/img.wgzimmer.ch/loading.gif",)
            )
        )
        ...
        logger.error(f"{e} - failed fetching  {url}")
